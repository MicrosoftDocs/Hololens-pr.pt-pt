...
description: Mantenha-se atualizado com todas as atualizações de cada novo HoloLens 2.
author: scooley
ms.author: scooley
manager: laurawi
ms.prod: hololens
...
- Problema fixo em que Wi-Fi proxy não pode ser salvo quando Wi-Fi propriedades UI é lançado a partir de **Definições > Network & Internet > Status > Properties**.
...
- Visite **Definições > System > Mixed Reality Camera** para permitir o "upload da câmara".
- Ao configurar esta funcionalidade para a posição **On,** quaisquer fotos ou vídeos de realidade mista capturados no seu dispositivo serão automaticamente enviados para o upload para a pasta Pictures > Camera Roll da sua OneDrive para trabalho ou conta escolar.
    >[!NOTE]
...
- Se a sua conta OneDrive tiver problemas (por exemplo, após a alteração da palavra-passe), aparecerá na página Definições um botão **'' Fix now).**
- Não existe o tamanho máximo do ficheiro, mas note que os ficheiros grandes demorarão mais tempo a ser carregados (especialmente se a largura de banda do upload estiver limitada). Se "parar" ou desligar o upload enquanto um ficheiro grande estiver a ser carregado, o upload parcial será preservado. Se o upload for re-activado dentro de algumas horas após ser "pausado" ou desligado, o upload continuará de onde parou. No entanto, se o upload for re-activado após várias horas, o upload do grande ficheiro recomeçará desde o início.

**Questões e ressalvas conhecidas**

- Esta definição não tem nenhuma configuração construída em estrangulamento com base no uso atual da largura de banda. Se precisar de maximizar a largura de banda para outro cenário, desligue a definição manualmente. O upload será interrompido, mas a funcionalidade continuará a monitorizar ficheiros recém-adicionados para o Camera Roll. Reesusibilizar o upload quando estiver pronto para que continue.
- Esta funcionalidade deve ser ativada para cada conta de utilizador no dispositivo, e apenas pode fazer o upload de ficheiros para o utilizador que se encontra atualmente inscrito no dispositivo.
...
- O upload fará uma pausa se o seu dispositivo adormecer ou estiver desligado. Para garantir que os uploads pendentes estão completos, utilize ativamente o dispositivo até que a página Definições se leia "OneDrive está atualizado" ou ajuste as definições **de energia & sono.**
...
- Corrige a grande corrupção de vídeo com a calibração de cor.
...
Esta atualização contém funcionalidades para dois públicos-alvo; funcionalidades que podem ser utilizadas por qualquer pessoa num dispositivo pelo Utilizador Final, e novas opções de gestão de dispositivos que podem ser configuradas por Administrações de TI. A tabela abaixo especifica as características que são relevantes para cada público. Se você é um administrador de TI, por favor dê uma olhada na nossa [lista de verificação de informação de TI - Update Checklist](#it-admin---update-checklist).
>[!IMPORTANT]
...
|-----------------------------------------------------------|----------------------------------------------------------------------------------------|--------------------|
| [Nova Microsoft Edge](#introducing-the-new-microsoft-edge)  | A nova Microsoft Edge baseada em Chromium está agora disponível para HoloLens 2. | Utilizador Final |
...
[Mostrar calibração de cores](#display-color-calibration) | Selecione um perfil de cor alternativo para o seu ecrã HoloLens 2. | Utilizador Final |
...
[Swipe para escrever](#swipe-to-type) | Use a ponta do dedo para "passar" palavras no teclado holográfico. | Utilizador Final |
...
[Pré-visualização da otimização de entrega](#delivery-optimization-preview) | Reduza o consumo de largura de banda para downloads de vários dispositivos HoloLens. | Administrador de TI |
...
#### <a name="launching-the-new-microsoft-edge"></a>Lançar o novo Microsoft Edge
...
> [!NOTE]
> Quando lançar pela primeira vez o novo Microsoft Edge no dia 2 HoloLens, as suas definições e dados serão importados de Microsoft Edge antigas. Se continuar a utilizar o legado Microsoft Edge depois de lançar o novo Microsoft Edge, esses novos dados não serão sincronizados desde o legado Microsoft Edge até ao novo Microsoft Edge.
...
Como o novo Microsoft Edge é uma aplicação nativa win32 com uma nova camada de adaptador UWP que lhe permite funcionar em dispositivos apenas UWP como HoloLens 2, algumas funcionalidades podem não estar imediatamente disponíveis. Vamos apoiar novos cenários e funcionalidades ao longo dos próximos meses, por isso, verifique este espaço para obter informações atualizadas.

**Cenários e características que se espera que funcionem:**

- Experiência de primeira execução, iniciar sessão no perfil e sincronizar
- Os sites devem renderizar e comportar-se como esperado
- A maioria das funcionalidades do navegador (Favoritos, Histórico, etc.) devem funcionar como esperado
- Dark mode (Modo escuro)
- Instalar aplicações web para o dispositivo
...
- Som espacial de uma única janela de navegador
...
- Restauração de conteúdos para corrigir janela, ao navegar em várias janelas colocadas no seu ambiente
...
- A pré-visualização do amplificador no teclado holográfico foi desativada para o novo Microsoft Edge. Esperamos reemencar esta funcionalidade numa futura atualização, uma vez que a ampliação está a funcionar corretamente.
- O áudio pode ser reproduzido a partir da janela do navegador errado se tiver outra janela do navegador aberta e ativa. Podes contornar este problema fechando a outra janela ativa que não é suposto estar a reproduzir áudio.
...
**Instalação direta no dispositivo (atualmente apenas disponível para dispositivos não geridos)**
  ...
  1. Na sua HoloLens 2, visite a [página de descarregamento do Edge Insider](https://www.microsoftedgeinsider.com/download).
  1. Selecione o botão **Descarregar para HoloLens 2** para o canal Edge Insider que pretende instalar.
  ...
  1. Após a instalação com sucesso, encontrará Microsoft Edge Beta, Dev ou Canário como uma entrada separada na lista de **aplicações de todas** as aplicações do menu Iniciar.
...
Alguns ambientes podem ter restrições de rede para contabilizar como uma consideração. Para garantir uma experiência suave com o novo Edge, por [favor, ative estes pontos finais da Microsoft.](/deployedge/microsoft-edge-security-endpoints)
...
Pode utilizar o novo Edge para instalar aplicações web ao lado de aplicações Microsoft Store. Por exemplo, pode instalar a aplicação web Microsoft Office para visualizar e editar ficheiros alojados no SharePoint ou OneDrive. Para instalar a Office aplicação web, visite https://www.office.com e selecione a **app Disponíveis** ou **instale o** botão Office na barra de endereços. **Selecione Instalar** para confirmar.
...
**Amostras recomendadas da WebXR**
...
Com esta versão, estamos a introduzir uma nova versão da aplicação Definições. A nova aplicação Definições inclui novas funcionalidades e configurações expandidas para HoloLens 2 nas seguintes áreas: Som, Energia & sono, Rede & Internet, Apps, Contas, Facilidade de Acesso e muito mais.
...
- Definições pesquisar: procure configurações a partir da página inicial Definições utilizando palavras-chave ou o nome da definição.
- Sistema > Som:
...
- Sistema > Bateria: ativar manualmente o modo de poupança de bateria ou definir um limiar de bateria em que o modo de poupança da bateria se liga automaticamente.
...
- Aplicações: pode redefinir as aplicações padrão utilizadas para tipos de ficheiros e ligações. Para obter mais informações consulte [o selecionador de aplicações padrão.](#default-app-picker)
- Contas > Outros utilizadores: os proprietários de dispositivos podem adicionar utilizadores, atualizar os utilizadores padrão aos proprietários do dispositivo, desvalorizar os proprietários de dispositivos para utilizadores padrão e remover os utilizadores.
...
Com esta nova definição, pode selecionar um perfil de cores alternativo para o seu ecrã HoloLens 2. Isto pode ajudar as cores a parecerem mais precisas, especialmente em níveis de luminosidade de ecrã mais baixos. A calibração da cor do ecrã pode ser encontrada na aplicação Definições, na página de Calibração > do Sistema.
...
##### <a name="how-to-use-display-color-calibration"></a>Como usar a calibração de cor do ecrã

1. Lançar a aplicação **Definições** e navegar para **a Calibração > Sistema.**
...
1. Quando estiver satisfeito com o perfil selecionado, selecione o botão **'Saída 'Guardar &'**
1. Se preferir não fazer alterações, selecione o botão **'Cancelar & Saída'** e as alterações serão revertidas

> [!TIP]
> Aqui ficam algumas dicas úteis a ter em mente enquanto utiliza a definição de calibração da cor do ecrã:
>
> - Pode re-executar a calibração da cor do ecrã a partir de Definições sempre que quiser
...
#### <a name="top-display-color-calibration-known-issues"></a>Principais problemas de calibração da cor do ecrã

- Na página Definições, a cadeia de estado que lhe diz quando o perfil de cor foi alterado ficará desatualizada até que recarregue a página de Definições.
...
Quando ativar uma hiperligação ou abrir um tipo de ficheiro com mais de uma aplicação instalada, que a suporta, verá uma nova janela aberta, levando-o a selecionar qual a aplicação instalada que deve lidar com o ficheiro ou o tipo de ligação. Nesta janela, também pode optar por ter a aplicação selecionada a lidar com o ficheiro ou o tipo de ligação "Uma vez" ou "Sempre".

Se escolher "Always" mas depois quiser alterar qual a aplicação que lida com um determinado ficheiro ou tipo de ligação, pode redefinir as suas predefinições guardadas em **Definições > Apps**. Desloque-se para a parte inferior da página e selecione o botão **'Limpar'** em "Aplicações predefinidos para tipos de ficheiros" e/ou "Aplicações predefinidos para tipos de ligação". Ao contrário da definição semelhante nos computadores de secretária, não é possível redefinir os predefinidos individuais do tipo de ficheiro.

#### <a name="per-app-volume-control"></a>Por controlo de volume de aplicações

Agora, nesta Windows construir, os utilizadores podem ajustar manualmente o nível de volume de cada aplicação. Isto permite que os utilizadores se concentrem melhor nas apps que precisam, ou melhor ouvir quando usam várias apps. Como a necessidade de baixar o volume de uma aplicação enquanto chama outra pessoa para assistência remota noutra.
...
#### <a name="swipe-to-type"></a>Swipe para escrever
...
Por favor, note que esta funcionalidade pode ser complicada de usar e dominar devido à natureza de um teclado holográfico onde não se sente resistência contra o dedo (ao contrário de um ecrã de telemóvel). 
...
> [!IMPORTANT]
> A ligação **num microfone USB não o definirá automaticamente como o dispositivo de entrada**. Ao ligar um conjunto de auscultadores USB-C, os utilizadores observarão que o áudio do auscultador será automaticamente redirecionado para os auscultadores, mas o HoloLens OS prioriza a matriz interna do microfone acima de qualquer outro dispositivo de entrada. **Para utilizar um microfone USB-C, siga os passos abaixo.**
...
>
> Em seguida, utilize o drop-down para definir o microfone externo como **Predefinido** ou **Padrão de Comunicações.** Escolher **o Padrão** significa que o microfone externo será utilizado em todo o lado.
>
...
![Descodei o microfone.](images/usbc-mic-3.jpg)
...
### <a name="use-the-new-settings-and-edge-apps-in-kiosk-modes"></a>Use as novas aplicações Definições e Edge nos modos Quiosque

Ao incluir aplicações em Quiosques, um administrador de TI [muitas vezes](hololens-kiosk.md)adiciona a aplicação ao Quiosque, mas utilizando o seu ID de Modelo de Utilizador de Aplicações (AUMID). Uma vez que tanto a aplicação Definições como Microsoft Edge aplicação são consideradas novas aplicações e diferentes das aplicações mais antigas, os quiosques que utilizam AUMIDs para essas aplicações terão de ser atualizados para utilizar o novo AUMID.

Ao modificar um Quiosque para incluir as novas aplicações, recomendamos adicionar o novo AUMID, bem como sair do antigo. Isto criará uma transição fácil quando os utilizadores atualizarem o SO e não precisarem de receber novas políticas para continuarem a utilizar o Quiosque como pretendido.

| Aplicação                    | AUMID                                                  |
|------------------------|--------------------------------------------------------|
| App old Definições       | HolographicSystemSettings_cw5n1h2txyewy! App            |
| Nova app Definições       | BAEAEF15-9BAB-47FC-800B-ACECAD2AE94B_cw5n1h2txyewy! App |
| App de Microsoft Edge antiga | Microsoft.MicrosoftEdge_8wekyb3d8bbwe! MicrosoftEdge    |
...
| Fronteiras de imagem de > de privacidade                         | `ms-settings:privacy-graphicsCaptureWithoutBorder` |
| Imagem de > de privacidade e aplicativos                       | `ms-settings:privacy-graphicsCaptureProgrammatic`  |
...
| System > Sound > As preferências de app e dispositivo | `ms-settings:apps-volume`                          |
| Sistema > > de som Gerencie dispositivos de som              | `ms-settings:sound-devices`                        |
| Configurar > Armazenamento > Armazenamento Sense         | `ms-settings:storagepolicies`                      |
...
Esta funcionalidade pode ser gerida através [da Conectividade/PermitirDevices.](/windows/client-management/mdm/policy-csp-connectivity#connectivity-allowconnecteddevices)
...
### <a name="delivery-optimization-preview"></a>Pré-visualização da otimização de entrega
...
Se anteriormente estava a usar a aplicação Definições ou Microsoft Edge app num Quiosque, substituímos estas aplicações por novas aplicações que utilizam um ID de aplicações diferente. Encorajamo-lo a ler [Novos AUMIDs para novas aplicações no modo Quiosque](#use-the-new-settings-and-edge-apps-in-kiosk-modes) abaixo. Isto irá garantir que ou continua a ter a aplicação Definições no seu Quiosque, ou inclui a nova app Microsoft Edge. Estas alterações podem ser feitas agora e implementadas em todos os dispositivos e permitem uma transição mais suave na atualização.
...
✔️ Partilhar[coisas com dispositivos próximos](#share-things-with-nearby-devices): Pode desativar a nova funcionalidade de partilha nas proximidades.
...
✔️[novos vestígios de diagnóstico do SO](#new-os-diagnostic-traces): Recolher registos relacionados com as atualizações do SISTEMA.
...
- Corrige um problema em torno da implementação de aplicações de linha de negócio através de pacotes de provisionamento de tempo de execução.
- Corrige um problema em torno da linha de aplicação de aplicação instalar relatórios de estado.
...
- Os clientes que usam AdvancedPhotoCapture ou LowLagPhotoCapture para capturar fotografias com HoloLens 2 conseguem agora recuperar a pose da câmara até 3 segundos após a foto ter sido capturada.
- Correção de uma fuga de memória no Serviço portal do dispositivo, o problema causou um aumento do uso da memória pelo serviço que fez com que outras aplicações falhassem na atribuição de memória.
...
Esta atualização mensal de qualidade não contém quaisquer alterações notáveis, encorajamos-te a experimentar as nossas mais recentes construções para Windows Holographic, versão 2004.
...
Estamos **a adicionar uma nova capacidade (App Installer) para permitir a instalação de aplicações de forma mais perfeita** nos seus HoloLens 2 dispositivos. A funcionalidade será **acesa por padrão para dispositivos não geridos**. Para evitar perturbações nas empresas, o instalador de aplicações **não estará disponível para dispositivos geridos** neste momento.  
...
Leia sobre o processo completo de instalação de [apps no dia 2 HoloLens com o Instalador de Aplicações.](app-deploy-app-installer.md)  

![Instalação de exemplos MRTK através do Instalador de Aplicações.](images/hololens-app-installer-picture.jpg)

### <a name="improvements-and-fixes-in-the-update"></a>Melhorias e correções na atualização:

- O rastreio manual mantém agora o rastreio em muitos casos novos em que a mão anteriormente teria sido perdida.  Em alguns destes novos casos, apenas a posição da palma da mão continua a ser atualizada com base na mão real do utilizador, enquanto as outras articulações são deduzidas com base numa pose anterior.  Esta mudança ajuda a melhorar a consistência do rastreio em movimentos como bater, atirar, colher e bater palmas.  Também ajuda nos casos em que a mão está perto de uma superfície ou segurando um objeto.  Quando as articulações manuais estiverem a ser inferidas, o valor [de precisão por articulação](/uwp/api/windows.perception.people.jointposeaccuracy?view=winrt-19041&preserve-view=true) será definido como "Aproximado" em vez de "Alto".
...
- Os utilizadores devem ter um fuso horário correto vindo do OOBE.
...
Esta atualização mensal de qualidade não contém quaisquer alterações notáveis, encorajamos-te a experimentar o nosso mais recente Windows Holographic, versão 20H2 – dezembro de 2020 e a nova funcionalidade De Instalador de Aplicações adicionada na construção.
...
| [Provisão de lançamento automático a partir de USB](hololens-release-notes.md#auto-launch-provisioning-from-usb)                    | O provisionamento de pacotes em unidades USB solicita automaticamente a página de provisionamento em OOBE.                                                         |
...
| [Acesso Global Atribuído](hololens-release-notes.md#global-assigned-access--kiosk-mode)                               | Novo método de configuração para o modo de quiosque de várias aplicações que aplica o quiosque ao nível do sistema, tornando-o aplicável a todos.                  |
...
| [Novas políticas de poder para HoloLens 2](hololens-release-notes.md#new-power-policies-for-hololens-2)       | Políticas recentemente apoiadas para definições de tempo limite de energia.  |
...
| [Visibilidade da página de Definições ativada para HoloLens 2](hololens-release-notes.md#enabled-settings-page-visibility-for-hololens-2)      | Política para escolher quais páginas são vistas em Definições app.             |
...
Se o utilizador transitar de uma aplicação ativada sem olhar para uma que aceda aos dados do olhar, o aviso de calibração será apresentado.

Todos os outros comportamentos do sistema serão semelhantes aos de quando o utilizador atual não tem uma calibração ativa de rastreio de olhos. Por exemplo, o gesto de arranque com uma mão não será ativado. Não haverá alteração da Experiência Fora da Caixa para a configuração inicial.
...
#### <a name="steps-to-install-a-certificate"></a>Passos para instalar um certificado
...
#### <a name="steps-to-remove-a-certificate"></a>Passos para remover um certificado
...
Antes desta versão, os utilizadores tiveram de lançar manualmente o ecrã de provisionamento durante o OOBE para obterem uma combinação de botões. Agora os utilizadores podem saltar a combinação de botões, utilizando um Pacote de Provisionamento numa unidade de armazenamento USB.
...
Informações adicionais sobre [o fornecimento automático de um USB](hololens-provisioning.md#auto-launch-provisioning-from-usb) podem ser encontradas na documentação de provisionamento HoloLens.

### <a name="auto-confirm-provisioning-packages-in-oobe"></a>Pacotes de provisão automática em OOBE

- O processo automatizado permite uma menor interação do utilizador, quando a página 'Pacote de Provisionamento' for exibida, aplicará automaticamente todos os pacotes listados.
...
1. Quando [o Advanced Recovery Companion](https://www.microsoft.com/store/productId/9P74Z35SFRS8) tiver terminado de piscar o seu dispositivo desligue o cabo USB-C.
...
[TenantLockdown](/windows/client-management/mdm/tenantlockdown-csp) A CSP permite que HoloLens 2 estejam ligados à inscrição de MDM usando apenas o Autopilot. Uma vez que o nó de RequernetworkInOOBE do TenantLockdown CSP seja definido para um valor verdadeiro ou falso (inicialmente definido) no HoloLens 2, esse valor permanece no dispositivo apesar de voltar a piscar, atualizações de SO, etc.
...
- Experiência focada no lançamento automático de aplicações, aumentando ainda mais as seleções de UI e aplicações escolhidas para experiências em modo quiosque.

Aplica-se apenas ao modo quiosque de várias aplicações e apenas 1 aplicação pode ser designada para lançamento automático usando o atributo realçado abaixo na configuração de Acesso Atribuído.
...
| MixedReality\MicrophoneDisabled                    | Desativa o microfone para que não seja possível gravar áudio HoloLens 2.                      | 1 Sim, 0 Não (padrão)                                                |
| Realidade Mista\FallbackDiagnostics                   | Controla o comportamento de quando os registos de diagnóstico podem ser recolhidos.                               | 0 Desativado, 1 Habilitado para Os Proprietários do Dispositivo, 2 Ativado para todos (Predefinido) |
| MixedReality\HeadTrackingMode                      | Reservado para uso futuro.                                                                  |                                                                      |
| MixedReality\AADGroupMembershipCacheValidityInDays | Controla quantos dias a cache de membro do grupo Azure AD é usada para quiosques direcionados para grupos AZure AD. | Veja abaixo.                                                           |
...
1. Deixe o utilizador Azure AD 1 iniciar sposição quando a internet estiver disponível, uma vez que a subscrição do grupo AD do utilizador e do Azure AD seja confirmada com sucesso, a cache será criada.
...
1. Os passos 4 e 5 podem ser repetidos para qualquer outro utilizador AZure AD N. O ponto chave aqui é que qualquer utilizador AD AD deve iniciar sação para dispositivo usando a Internet para que pelo menos uma vez que possamos determinar que eles são membros do grupo AD AZure para o qual a configuração do Quiosque é alvo.

> [!NOTE]
> Até que o passo 4 seja realizado para um utilizador AZure AD irá experimentar o comportamento de falha mencionado em ambientes "desligados".
...
|----------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
|     [DisplayOffTimeoutOnBattery](/windows/client-management/mdm/policy-csp-power#power-displayofftimeoutonbattery)               |     Valor de exemplo a utilizar no Windows Designer de Configuração, ou seja,`<enabled/><data   id="EnterVideoDCPowerDownTimeOut" value="100"/>`     |
|     [DisplayOffTimeoutPluggedIn](/windows/client-management/mdm/policy-csp-power#power-displayofftimeoutpluggedin)               |     Valor de exemplo a utilizar no Windows Designer de Configuração, ou seja,`<enabled/><data   id="EnterVideoACPowerDownTimeOut" value="100"/>`     |
...
### <a name="enabled-settings-page-visibility-for-hololens-2"></a>Visibilidade da página de Definições ativada para HoloLens 2
...
Devido ao feedback do cliente, aumentámos o comprimento de gravação das capturas de [realidade mista.](holographic-photos-and-videos.md) As capturas de realidade mista deixarão de ser limitadas a 5 minutos por defeito, mas calcularão o comprimento máximo de gravação com base no espaço disponível do disco. O dispositivo estimará a duração máxima da gravação de vídeo com base no espaço disponível do disco até 80% do espaço total do disco.

> [!NOTE]
> O HoloLens utilizará o comprimento de gravação de vídeo predefinido (5 minutos) se ocorrer um dos seguintes:
>
> - A duração máxima estimada de gravação é menor do que a de padrão de 5 minutos.
...
### <a name="improvements-and-fixes-in-the-windows-holographic-version-20h2-update"></a>Melhorias e correções na Windows holográfica, versão 20H2:
...
- Corrigiu um problema devido à interferência do caching HTTP com o modo quiosque direcionado para os grupos AZure AD.
- Corrigiu um problema em que os utilizadores não podiam utilizar o botão Par depois de permitirem o modo desenvolvedor com pacotes de provisionamento, a menos que tenham desativado e reativado o modo de Desenvolvedor.

## <a name="windows-holographic-version-1903---november-2020-update"></a>Windows Holográfico, versão 1903 - Atualização de novembro de 2020

- Construção 18362.1085

Esta atualização mensal de qualidade não contém quaisquer alterações notáveis, encorajamos-te a experimentar a nossa mais recente versão de lançamento Windows Holographic, versão 20H2.
...
## <a name="windows-holographic-version-1903---october-2020-update"></a>Windows Holográfico, versão 1903 - Atualização de outubro de 2020
...
- Aborda um problema que impediu Visual Studio de depurar uma aplicação quando o SupportsMultipleInstances="true" está presente na appxmanifest.
...
- Definições aplicação deixará de seguir o utilizador para experiências de Inscrição de Íris ou Calibração de Rastreio de Olhos.
- Fixar um bug quando aplicar um pacote de provisionamento durante o OOBE que rebatiza o dispositivo e executa outras ações (como a ligação a uma rede) não executaria as outras ações após o reboot do dispositivo devido ao renomeamento.
...
- Os gravadores MRC personalizados têm agora novos valores predefinidos para determinadas propriedades se não forem especificados.
  ...
  - Sobre o *efeito áudio MRC:*
    - LoopbackGain (o valor atual de "Ganho de Áudio de Aplicação" na página de captura de realidade mista no Portal Windows dispositivo)
...
- A API HolographicSpace.UserPresence é geralmente desativada para aplicações de unidade. Este comportamento evita um problema que fez com que algumas aplicações parassem quando a viseira foi virada para cima, mesmo que a definição de "correr em segundo plano" estivesse ativada. A API está agora ativada para as versões Unidade 2018.4.18 e posterior e 2019.3.4 e posterior.
...
- Corrigiu um problema que fez com que HoloLens aplicações que alteram o seu formato de pixels se tornem pretas no emulador HoloLens.
...
- Corrigiu um problema que fez com que HoloLens aplicações que alteram o seu formato de pixels se tornassem pretas no HoloLens Emulator.
...
A grande atualização de software de maio de 2020 para HoloLens 2, *Windows a versão Holográfica, versão 2004* inclui uma série de novas capacidades interessantes, como suporte para Windows Autopilot, modo escuro de aplicação, suporte USB Ethernet para hotspots 5G/LTE, e muito mais. Para atualizar a versão mais recente, abra a   aplicação Definições, vá ao **Update & Security** e selecione o botão Check for **Updates.**   
...
|       Estado de instalação de aplicação                 |          Verifique o estado da instalação na aplicação Definições para apps foram empurradas para HoloLens 2 via MDM               |
...
1. Utilize o Azure AD para inscrever o dispositivo em Microsoft Intune (ou noutro serviço MDM).
1. Descarregue as políticas, certificados e perfis de rede direcionados para o dispositivo.
1. A provisiona o dispositivo.
...
Para começar, consulte [Ativar o início do sing-in da chave de segurança sem palavras-passe](/azure/active-directory/authentication/howto-authentication-passwordless-security-key).

### <a name="improved-mdm-enrollment-via-provisioning-package"></a>Inscrição melhorada do MDM através de pacote de provisionamento

Os pacotes de provisionamento permitem-lhe definir HoloLens configuração através de um ficheiro config e não através da experiência fora de caixa HoloLens. Anteriormente, os pacotes de provisionamento tinham de ser copiados para a HoloLens memória interna. Agora podem estar numa unidade USB para que sejam mais fáceis de reutilizar em vários dispositivos HoloLens e você pode providenciar dispositivos em paralelo. Os pacotes de provisionamento agora também suportam um campo para se inscrever na gestão do dispositivo para que não haja configuração manual após o provisionamento.
...
1. Construa o seu perfil de configuração. Em seguida, copie todos os ficheiros que foram criados para um dispositivo de armazenamento USB-C.
1. Ligue o dispositivo USB-C a qualquer HoloLens recentemente intermitente. Em seguida, pressione os botões de potência **para baixo** para aplicar o seu pacote de  +   provisionamento.

### <a name="line-of-business-application-install-status"></a>Estado de instalação de aplicações em linha de negócio

A implementação e gestão de aplicações de MDM para aplicações de linha de negócios é fundamental para HoloLens. Os administradores e utilizadores precisam de visualizar o estado de instalação da aplicação para auditoria e diagnóstico. Nesta versão, adicionámos mais detalhes no **Definições**  >  **Contas**  >  **Access ou escola Clique** na sua  >  **conta**  >  **Informações.**
...
- **Roubo de mãos reduzida:** Melhoramos o tratamento dos casos em que há mais de duas mãos em vista dos sensores. Se várias pessoas estão a trabalhar juntas, há agora uma hipótese muito menor de que a mão rastreada "salte" do utilizador para a mão de outra pessoa na cena do crime.
...
- Corrigi um bug onde Wi-Fi a HoloLens foi interrompido periodicamente. Se uma aplicação indicar que necessita de um streaming de latência baixa, implemente a correção chamando a [função SetSocketMediaStreamingMode](/windows/win32/api/socketapi/nf-socketapi-setsocketmediastreamingmode).
...
- Corrigiu um problema em que o subsistema de rastreio ocular não iniciaisou ou efetuou a calibração em determinadas condições.
...
- Adicionou um botão **Partilhar** no Centro de Comentários para que os utilizadores possam partilhar mais facilmente o feedback.
...
## <a name="windows-holographic-version-1903---april-2020-update"></a>Windows Holográfico, versão 1903 - Atualização abril 2020
...
- Assegurou-se que as sobreposições da concha estão incluídas em capturas de realidade mista.
...
- Melhor estabilidade do holograma na captura de realidade mista quando o algoritmo *HolographicDepthReprojection MethPlanar* é usado.
...
- Corrigiu um acidente aleatório de HUP causado por um rastreio manual, no qual o utilizador notou um congelamento de UI e depois voltou a ser bombardeado após vários segundos.
- Um rastreio de mão melhorado para que quando você pica com o seu dedo indicador, a parte superior desse dedo é menos propenso a enrolar inesperadamente.
- Melhor fiabilidade do rastreio da cabeça, mapeamento espacial e outros tempos de funcionamento.
...
## <a name="windows-holographic-version-1903---december-2019-update"></a>Windows Holográfico, versão 1903 - Atualização de dezembro 2019

- Construção 18362.1042

Melhorias e correções na atualização:

- Introduziu correções de reprodução de última fase (LSR). Uma melhor renderização visual dos hologramas parece mais estável e crispada, contabilizando com mais precisão a sua profundidade. Este sintoma será mais percetível após esta atualização se as aplicações não definirem corretamente a profundidade dos hologramas.
...
- Correcçãos e melhorias de conformidade de funcionamento do OpenXR para HoloLens 2 e para o emulador.
