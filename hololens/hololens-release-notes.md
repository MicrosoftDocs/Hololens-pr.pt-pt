...
>[!IMPORTANT]
>Para atualizar esta construção, os dispositivos HoloLens 2 devem estar atualmente a executar a atualização de fevereiro de 2021 (build 19041.1136) ou mais recente. Se não estiver a ver esta atualização disponível, por favor atualize o seu dispositivo primeiro e tente novamente.

>[!NOTE]
>Hoje, o Microsoft HoloLens 2 suporta atualizações mensais de manutenção (correções de erros e segurança) para as seguintes versões:
>- Windows Holographic, versão 20H2 (Construção 19041.1128+)
>- Windows Holographic, versão 2004 (Build 19041.1103+)
>- Windows Holographic, versão 1903 (Build 18362+) 
>
> Com a introdução da versão Holográfica 21H1 do Windows, **estamos a descontinuar as atualizações mensais de manutenção para a versão Holográfica do Windows 1903**. Isto permite-nos focar em lançamentos mais recentes e continuar a oferecer melhorias valiosas. 


| Nome do recurso                                              | Breve descrição                                                                      | Público-alvo | 
|-----------------------------------------------------------|----------------------------------------------------------------------------------------|--------------------|
| [Novo Microsoft Edge](#introducing-the-new-microsoft-edge)  | O novo Microsoft Edge baseado em Chromium já está disponível para HoloLens 2. | Utilizador Final | 
...
[App Novas Definições](#new-settings-app) | A aplicação Definições antigas está a ser substituída por uma versão atualizada com novas funcionalidades e configurações. | Utilizador Final |
...
[Instalar aplicativos web](#install-web-apps) | Instale aplicações web no HoloLens 2, como o Microsoft Office, com o novo navegador Microsoft Edge. | Utilizador Final |
[Passe para escrever](#swipe-to-type) | Use a ponta do dedo para "passar" palavras no teclado holográfico. | Utilizador Final |
[Menu de energia a partir do início](#power-menu-from-start) | No Menu Iniciar, reinicie e desligue o dispositivo HoloLens. | Utilizador Final |
...
[Novos AUMIDs para novas apps em modo Quiosque](#use-the-new-settings-and-edge-apps-in-kiosk-modes)  | AUMIDs para novas configurações e aplicações Edge. | Administrador de TI |
...
[Novas definiçõesURIs para visibilidade de definições de página](#new-settings-uris-for-page-settings-visibility) | 20+ nova política de Definições/PageVisibilityList. | Administrador de TI |
[Configurar diagnósticos de recuo](#configuring-fallback-diagnostics-via-settings-app) | Definição do comportamento de diagnóstico do recuo na aplicação De definições. | Administrador de TI |
[Partilhar coisas com dispositivos próximos](#share-things-with-nearby-devices) | Partilhe ficheiros ou URLs de um HoloLens para um PC. | Todos |
[Novos vestígios de diagnóstico de SO](#new-os-diagnostic-traces) | Novo resolução de problemas em Definições para atualizações de SO. | Administrador de TI |
...
- [Visite o arquivo emulador HoloLens](https://docs.microsoft.com/windows/mixed-reality/hololens-emulator-archive)
...
> [!NOTE]
> Quando lançar pela primeira vez o novo Microsoft Edge no HoloLens 2, as suas definições e dados serão importados do legado Microsoft Edge. Se continuar a utilizar o legado Microsoft Edge depois de lançar o novo Microsoft Edge, os novos dados não serão sincronizados do legado microsoft Edge para o novo Microsoft Edge.
...
Com esta versão, estamos a introduzir uma nova versão da aplicação Settings. A nova aplicação Definições inclui novas funcionalidades e definições expandidas para HoloLens 2 nas seguintes áreas: Som, Energia & sono, Rede & Internet, Apps, Contas, Facilidade de Acesso e muito mais.
...
Com esta nova definição, pode selecionar um perfil de cores alternativo para o seu ecrã HoloLens 2. Isto pode ajudar as cores a parecerem mais precisas, especialmente em níveis de luminosidade de ecrã mais baixos. A calibração de cores do ecrã pode ser encontrada na aplicação Definições, na página de calibração system >.
...
1. Inicie a aplicação **Definições** e navegue para **a Calibração > Sistema.**
...
1. Depois de proceder através das caixas de diálogo de instruções, o seu visor será automaticamente diminuído para 30% de luminosidade.
    > [!TIP]
    > Se tiver dificuldades em ver a cena escurecida no seu ambiente, pode ajustar manualmente o nível de luminosidade dos HoloLens 2 utilizando os botões de luminosidade no lado esquerdo do dispositivo.
...
> - Pode re-executar a calibração de cores do ecrã a partir de Definições sempre que quiser
...
> - Se pretender reverter para o perfil de cor original do visor, pode fazê-lo a partir da página Definições (ver [como redefinir](#how-to-reset-color-profile)o perfil de cor )

##### <a name="how-to-reset-color-profile"></a>Como redefinir o perfil de cor 

Se estiver insatisfeito com o perfil de cor personalizado guardado para os HoloLens 2, pode restaurar o perfil de cor original do dispositivo:
1. Inicie a aplicação **Definições** e navegue para **a Calibração > Sistema.**
...
- Na página Definições, a cadeia de estado que lhe diz quando o perfil de cor foi alterado ficará desatualizada até recarregar a página de Definições.
    - Solução: Selecione outra página de Definições e, em seguida, reescorente a página de Calibração.
...
Um novo menu que permite ao utilizador assinar, desligar e reiniciar o dispositivo. Um indicador no ecrã Iniciar HoloLens que mostra quando uma atualização do sistema está disponível.
...
5. Selecione as opções do menu para iniciar, reiniciar ou desligar os HoloLens. A opção Sign out pode não estar disponível, se o dispositivo estiver configurado para uma [única Conta Microsoft (MSA) ou conta local](hololens-identity.md).
...
Introduzido nesta construção do Windows, ao selecionar **Outro utilizador** que se encontra localizado à direita do campo de entrada PIN, o Sinal no ecrã apresentará vários utilizadores com o qual já se tenham inscrito no dispositivo. Isto permite que os utilizadores selecionem o seu perfil de utilizador e, em seguida, inscrevam-se utilizando as suas credenciais Windows Hello. Um novo utilizador também pode ser adicionado ao dispositivo a partir desta página de Outros utilizadores através do botão **de conta Adicionar.**
...
Infelizmente, os microfones Bluetooth ainda não estão suportados atualmente nos HoloLens 2.
...
### <a name="use-the-new-settings-and-edge-apps-in-kiosk-modes"></a>Utilize as novas aplicações Definições e Edge nos modos quiosque

Ao incluir aplicações em quiosques, um administrador de TI [muitas vezes](hololens-kiosk.md)adiciona a aplicação ao Quiosque, mas utilizando o seu ID de Modelo de Utilizador de Aplicações (AUMID). Uma vez que tanto a aplicação Settings como a aplicação Microsoft Edge são consideradas novas aplicações e diferentes das aplicações mais antigas, os quiosques que utilizam AUMIDs para essas aplicações terão de ser atualizados para utilizar o novo AUMID.
...
| Aplicação                    | AUMID                                                  |
|------------------------|--------------------------------------------------------|
| App de Configurações Antigas       | HolographicSystemSettings_cw5n1h2txyewy! App            |
| App de Novas Definições       | BAEAEF15-9BAB-47FC-800B-ACECAD2AE94B_cw5n1h2txyewy! App |
| Antiga aplicação microsoft edge | Microsoft.MicrosoftEdge_8wekyb3d8bbwe! MicrosoftEdge    |
...
A partir desta versão do Windows, a experiência do quiosque irá recorrer à configuração global do quiosque (se presente) em caso de falhas durante o modo quiosque do grupo AAD.

### <a name="new-settings-uris-for-page-settings-visibility"></a>Novas configurações URIs para visibilidade de definições de página
...
Se visitar [a Visibilidade das Definições de Página,](settings-uri-list.md)pode encontrar instruções para utilizar este CSP e a lista de URIs disponíveis em versões anteriores.

Estamos a expandir a lista de URIs de Definições disponíveis, que os administradores de TI podem gerir. Alguns destes URIs são para áreas recém-disponíveis dentro da nova aplicação Definições. Se estiver a utilizar a política Definições/PageVisibilityList, reveja a seguinte lista e ajuste as páginas permitidas ou bloqueadas conforme necessário.
...
| Sistema > armazenamento > configurar sentido de armazenamento         | `ms-settings:storagepolicies`                      |
...
### <a name="configuring-fallback-diagnostics-via-settings-app"></a>Configurar diagnósticos de retorno de outono através da aplicação Settings

Agora, na App Definições, um utilizador pode configurar o comportamento dos [Diagnósticos de Retorno De Fallback](hololens-diagnostic-logs.md). Na aplicação Definições navegue para a página **de**  ->  **resolução de problemas de** privacidade para configurar esta definição.
...
Partilhe coisas com dispositivos próximos do Windows 10, incluindo computadores e outros dispositivos HoloLens 2. Pode experimentá-lo em **Definições De**  ->  **Experiências** Partilhadas do Sistema de  ->  **Definições** para partilhar ficheiros ou URLs de um HoloLens para um PC. Para mais detalhes leia mais sobre como [partilhar coisas com dispositivos próximos no Windows 10](https://support.microsoft.com/windows/share-things-with-nearby-devices-in-windows-10-0efbfe40-e3e2-581b-13f4-1a0e9936c2d9).
...
Para além dos anteriores resoluções de problemas dentro da aplicação Definições, foi adicionado um novo resolução de problemas com a adição da nova aplicação Definições para Atualizações do OS. Navegue para **Definições**  ->  **Atualizar &amp; atualização de segurança** Falha  >  **do** Windows  >  **Update** e selecione **Start**. Isto permite-lhe recolher vestígios enquanto reproduz o seu problema com as Atualizações de OS para ajudar melhor na resolução de problemas com o seu TI ou suporte.

### <a name="delivery-optimization-preview"></a>Pré-visualização da otimização de entregas

Com esta atualização HoloLens, o Windows Holographic for Business permite configurações de otimização de entrega para reduzir o consumo de largura de banda para downloads de vários dispositivos HoloLens. Uma descrição mais completa desta funcionalidade juntamente com a configuração recomendada da rede está disponível aqui: [Otimização de entrega para atualizações do Windows 10](https://docs.microsoft.com/windows/deployment/update/waas-delivery-optimization).
...
Se anteriormente estava a usar a aplicação Settings ou a app Microsoft Edge num Quiosque, substituímos estas aplicações por novas aplicações que utilizam um ID de aplicação diferente. Encorajamo-lo a ler [Novos AUMIDs para novas aplicações no modo Quiosque](#use-the-new-settings-and-edge-apps-in-kiosk-modes) abaixo. Isto irá garantir que ou continua a ter a aplicação Definições no seu Quiosque, ou inclui a nova aplicação Microsoft Edge. Estas alterações podem ser feitas agora e implementadas em todos os dispositivos e permitem uma transição mais suave na atualização.
...
#### <a name="updates-to-page-settings-visibility"></a>Atualizações para Visibilidade das Definições de Página

✔️ novas [**configurações URIs para visibilidade de definições de página**](#new-settings-uris-for-page-settings-visibility)

Se estiver atualmente a utilizar a [visibilidade das Definições de Página,](settings-uri-list.md) poderá desejar escrutinar os seus URIs existentes que permitiu ou bloqueou.

#### <a name="updates-for-your-wdac-policy"></a>Atualizações para a sua política WDAC
✔️ Se esteve a bloquear previamente o Microsoft Edge via WDAC, vai querer atualizar a sua política WDAC. Por favor, reveja o seguinte e use o código de amostra fornecido.
...
✔️ [Configurar as definições de política para o novo Microsoft Edge](#configuring-policy-settings-for-the-new-microsoft-edge): Reveja as novas configurações disponíveis para o Microsoft Edge.
...
- Aborda uma fiabilidade da atualização com impacto na instalação do Microsoft Edge. 
...
## <a name="windows-holographic-version-20h2---april-2021-update"></a>Windows Holographic, versão 20H2 - Atualização abril 2021
...
## <a name="windows-holographic-version-1903---april-2021-update"></a>Windows Holographic, versão 1903 - Atualização abril 2021
...
## <a name="windows-holographic-version-20h2---march-2021-update"></a>Windows Holographic, versão 20H2 - março 2021 Atualização
...
## <a name="windows-holographic-version-1903---march-2021-update"></a>Windows Holographic, versão 1903 - março 2021 Atualização
...
- Aborda um problema em torno de upgrades e voos para lançamentos posteriores do HoloLens.
- Certificados pré-instalados não instalados da loja de raiz eSIM dos dispositivos HoloLens.
...
Esta atualização mensal de qualidade não contém quaisquer alterações notáveis, encorajamos-te a experimentar as nossas mais recentes construções para o Windows Holographic, versão 2004.

## <a name="windows-holographic-version-20h2---january-2021-update"></a>Windows Holographic, versão 20H2 - Atualização janeiro 2021
...
## <a name="windows-holographic-version-1903---january-2021-update"></a>Windows Holographic, versão 1903 - Atualização janeiro de 2021
...
1.  Certifique-se de que está assinado no dispositivo HoloLens 2
1.  No seu PC, navegue para a sua aplicação personalizada e copie o seu appapp.appxbundle para o seu devicename\Internal Storage\Downloads.   Depois de ter terminado de copiar o seu ficheiro, pode desligar o seu dispositivo
...
Pode encontrar aplicações de amostra no [Windows Universal Samples GitHub](https://github.com/microsoft/Windows-universal-samples/tree/master/Samples) para testar este fluxo.
...
| [Políticas hololens](hololens-release-notes.md#hololens-policies)                                    | Novas políticas para hololens.     |
...
| [Novas políticas de restrição de dispositivos para HoloLens 2](hololens-release-notes.md#new-device-restriction-policies-for-hololens-2)       | As políticas de gestão de dispositivos ativaram recentemente o HoloLens 2.                                                                                |
...
| [Visibilidade da página de Definições Ativadas para HoloLens 2](hololens-release-notes.md#enabled-settings-page-visibility-for-hololens-2)      | Política para escolher quais páginas são vistas na aplicação Definições.             |
| [Modo de investigação](hololens-release-notes.md#research-mode) | Utilizar o modo de pesquisa no HoloLens 2. |
...
Para obter mais informações sobre a aplicação de pacotes de provisionamento durante o OOBE, visite a documentação [de provisionamento hololens.](hololens-provisioning.md#apply-a-provisioning-package-to-hololens-during-setup)
...
Ao combinar o lançamento automático do fornecimento de dispositivos USB e a confirmação automática de pacotes de provisionamento, um utilizador pode providenciar automaticamente dispositivos HoloLens 2 sem utilizar o UI do dispositivo ou até mesmo usar o dispositivo. Pode continuar a utilizar a mesma unidade USB e pacote de provisionamento para vários dispositivos. Isto é útil para a implementação de vários dispositivos ao mesmo tempo na mesma área. 

1. [Crie um pacote de provisionamento](hololens-provisioning.md) utilizando [o Designer de Configuração do Windows](https://www.microsoft.com/store/productId/9NBLGGH4TX22). 
...
1. Quando o dispositivo HoloLens 2 entrar no OOBE, detetará automaticamente o pacote de provisionamento na unidade USB e lançará a página de provisionamento.
...
Os dispositivos HoloLens 2 suportam agora o TenantLockdown CSP a partir da [versão Holográfica do Windows 20H2](hololens-release-notes.md#windows-holographic-version-20h2). 

[TenantLockdown](https://docs.microsoft.com/windows/client-management/mdm/tenantlockdown-csp) O CSP permite que os HoloLens 2 estejam ligados à inscrição de MDM usando apenas o Autopilot. Uma vez que o nó requere do CSP do TenantLockdown CSP é definido para o valor verdadeiro ou falso (inicialmente definido) no HoloLens 2, esse valor permanece no dispositivo apesar de voltar a piscar, atualizações de SO, etc. 

Uma vez que o nó RequernetworkInOOBE do TenantLockdown CSPs é definido para ser verdadeiro no HoloLens 2, o OOBE aguarda indefinidamente para que o perfil do Piloto Automático seja descarregado e aplicado com sucesso, após a conectividade da rede. 

Uma vez que o nó de RequerenetworkInOOBE do TenantLockdown CSPs seja definido para ser verdadeiro nos HoloLens 2, as seguintes operações são proibidas em OOBE: 
...
1. Faça com que o dispositivo HoloLens 2 seja membro do grupo criado em sincronização de passos e gatilhos anteriores.  
...
#### <a name="how-to-unset-tenantlockdowns-requirenetworkinoobe-on-hololens-2-using-intune"></a>Como desaparati o TenantLockdown's RequireNetworkInOOBE em HoloLens 2 usando Intune? 
1. Remova os HoloLens 2 do grupo de dispositivos ao qual a configuração do dispositivo acima foi atribuída anteriormente. 
...
#### <a name="what-would-happen-during-oobe-if-autopilot-profile-is-unassigned-on-a-hololens-after-tenantlockdown-was-set-to-true"></a>O que aconteceria durante o OOBE, se o perfil do Piloto Automático não fosse atribuído a um HoloLens depois do TenantLockdown ter sido definido como verdadeiro? 
...
Esta nova funcionalidade permite que um Administrador de TI configuure um dispositivo HoloLens 2 para vários quiosques de aplicações, que é aplicável a nível do sistema, não tem qualquer afinidade com qualquer identidade no sistema e aplica-se a todos os que assinam no dispositivo. Leia sobre esta nova funcionalidade em detalhe no [quiosque de acesso global holoLens.](hololens-global-assigned-access-kiosk.md)
...
Anteriormente, ao encontrar falhas na aplicação do modo quiosque, os HoloLens costumavam aparecer todas as aplicações no menu inicial. Agora na versão Holográfica do Windows 20H2, no caso de falhas, nenhuma aplicação será mostrada no menu inicial como abaixo: 

![Imagem do que o modo Quiosque agora parece quando falha.](images/hololens-kiosk-failure-behavior.png )

### <a name="hololens-policies"></a>Políticas hololens
- Opções de gestão de dispositivos especificamente para HoloLens criadas para gerir o dispositivo. 

Foram criadas novas políticas de realidade mista para dispositivos HoloLens 2 na versão Holográfica do Windows 20H2. As novas definições controláveis incluem: luminosidade de definição, volume de definição, gravação de áudio incapacitante em capturas de realidade mista, definição quando os diagnósticos podem ser recolhidos e cache de membro do grupo AAD.  

| Nova política hololens                                | Description                                                                               | Notas                                                                |
|----------------------------------------------------|-------------------------------------------------------------------------------------------|----------------------------------------------------------------------|
...
1. Crie um perfil de configuração do dispositivo para quiosques direcionados para os grupos AZure AD e atribua-o aos dispositivos HoloLens. 
1. Crie uma configuração personalizada do dispositivo baseado em OMA URI que define este valor de política para o número de dias desejado (> 0) e atribua-o ao dispositivo(s) HoloLens. 
...
1. Inscreva os dispositivos HoloLens e verifique se ambas as configurações são aplicadas no dispositivo. 
...
1. Agora, o utilizador AD AD 1 pode desligar os HoloLens e usá-lo para o modo quiosque, desde que o valor da política permita o número de dias X. 
...
Políticas recentemente ativadas que permitem mais opções de gestão de dispositivos HoloLens 2. 
...
> [!NOTE]
> No que diz respeito ao [RemoteLock,](https://docs.microsoft.com/windows/client-management/mdm/remotelock-csp)os HoloLens apenas suportam a configuração ./Fornecedor/MSFT/RemoteLock/Lock. As configurações que lidam com PIN como reset e recuperação não são suportadas.
...
Estas políticas de atualização estão agora ativadas nos dispositivos HoloLens 2:
...
Todos os detalhes sobre estas políticas de atualização e como usá-las para dispositivos HoloLens podem ser lidos aqui em [atualizações do Manage HoloLens](hololens-updates.md).

### <a name="enabled-settings-page-visibility-for-hololens-2"></a>Visibilidade da página de Definições Ativadas para HoloLens 2
- Controlo de UI aumentado na App de Definições, que pode ser confundido para mostrar uma seleção limitada de páginas.

Agora, permitimos que uma política que permite aos administradores de TI impedir que páginas específicas na aplicação De Definições do Sistema sejam visíveis ou acessíveis, ou para fazê-lo para todas as páginas, exceto as especificadas. Para aprender a personalizar totalmente esta funcionalidade clique no link abaixo.

- [PageVisibilityList](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-settings#settings-pagevisibilitylist)

Para saber quais as definições de página que pode personalizar no HoloLens 2, visite a [nossa página URIs de Definições](settings-uri-list.md). 
...
Enquanto em Modo de Pesquisa, o HoloLens 2 torna-se uma ferramenta potente para a pesquisa de visão computacional. Em comparação com edições anteriores, o Modo de Investigação para HoloLens 2 tem as seguintes vantagens:
...
> [!NOTE]
> Os HoloLens utilizarão o comprimento de gravação de vídeo predefinido (5 minutos) se ocorrer um dos seguintes:
...
- Corrigiu um problema em que os ícones no menu Iniciar foram dimensionado corretamente no modo quiosque.
...
Esta atualização mensal de qualidade não contém quaisquer alterações notáveis, encorajamos-te a experimentar a nossa mais recente versão de lançamento do Windows Holographic, versão 20H2.

## <a name="windows-holographic-version-2004---october-2020-update"></a>Windows Holographic, versão 2004 - outubro 2020 Atualização
...
## <a name="windows-holographic-version-2004---september-2020-update"></a>Windows Holographic, versão 2004 - setembro 2020 Atualização
- Construção 19041.1117

Melhorias e correções na atualização:

- Aborda um problema que impediu o Visual Studio de depurar uma aplicação quando supportsMultipleInstances="true" está presente na appxmanifest.
...
- Na maioria dos dispositivos de Realidade Mista do Windows, o vetor de direção dianteira é paralelo ao solo quando a cabeça do utilizador está numa posição neutra olhando para a frente. No entanto, versões anteriores do HoloLens 2 alinharam o vetor para ser perpendicular aos painéis de exibição, que é inclinado para baixo alguns graus em relação à orientação ideal. Versões mais recentes do HoloLens 2 corrigiram isto para garantir a consistência semântica entre fatores de forma.
...
## <a name="windows-holographic-version-2004---august-2020-update"></a>Windows Holographic, versão 2004 - agosto 2020 Atualização
- Construção 19041.1113

Melhorias e correções na atualização:

- A aplicação de definições deixará de seguir o utilizador para experiências de Inscrição de Íris ou Calibração de Rastreio de Olhos.
...
## <a name="windows-holographic-version-1903---august-2020-update"></a>Windows Holographic, versão 1903 - agosto 2020 Atualização
- Construção 18362.1074

Esta atualização mensal de qualidade não contém quaisquer alterações notáveis, encorajamos-te a experimentar as nossas mais recentes construções para o Windows Holographic, versão 2004.

## <a name="windows-holographic-version-2004---july-2020-update"></a>Windows Holographic, versão 2004 - julho 2020 Atualização
...
- Abordou um problema sobre o encaminhamento https para o Portal do Dispositivo Windows no HoloLens 2.

## <a name="windows-holographic-version-1903---july-2020-update"></a>Windows Holographic, versão 1903 - julho 2020 Atualização
...
- Corrigiu um problema que fez com que as aplicações exclusivas do HoloLens voltassem a cair na casca enquanto utilizavam o Emulator HoloLens com aceleração de hardware em determinados dispositivos.
- Abordou um problema relativo ao encaminhamento do HTTPS para o Portal do Dispositivo Windows no HoloLens 2.

## <a name="windows-holographic-version-2004---june-2020-update"></a>Windows Holographic, versão 2004 - junho 2020 Atualização
...
- Os gravadores MRC personalizados têm agora novos valores predefinidos para determinadas propriedades se não forem especificados.
  ...
  - Sobre o *efeito áudio MRC:*
    - LoopbackGain (o valor atual "App Audio Gain" na página de captura de realidade mista no Portal do Dispositivo Windows)
    - MicrofoneGain (o valor atual "Mic Audio Gain" na página de captura de realidade mista no Portal do Dispositivo Windows)
...
- Adicionou uma definição nos Hologramas do Sistema **de**  >    >  **Definições** que permite aos utilizadores remover automaticamente todos os hologramas da Casa da Realidade Mista quando o dispositivo é desligado.
- Corrigiu um problema que fez com que as aplicações HoloLens que alterassem o seu formato de pixels se tornassem pretas no emulador HoloLens.
...
- Corrigiu um bug para evitar que aplicações imersivas abrissem o Microsoft Edge repetidamente.
...
## <a name="windows-holographic-version-1903---june-2020-update"></a>Windows Holographic, versão 1903 - junho 2020 Atualização
...
- Corrigiu um problema que fez com que as aplicações HoloLens que alterassem o seu formato de pixel se tornassem pretas no Emulator HoloLens.
...
A grande atualização de software de maio de 2020 para HoloLens 2, *Windows Holographic, versão 2004* inclui uma série de novas capacidades interessantes, como suporte para Windows Autopilot, modo escuro de aplicação, suporte USB Ethernet para hotspots 5G/LTE, e muito mais. Para atualizar a versão mais recente, abra a aplicação **Definições,**   aceda à  **Atualização & Segurança**, e selecione o botão Check for  **Updates.**   

|             Funcionalidade                              |          Descrição                                                                                              |
|--------------------------------------------------|-------------------------------------------------------------------------------------------------------------------|
|       Windows Autopilot                          |          Pré-configuração e configuração perfeita de novos dispositivos para produção utilizando o Windows AutoPilot                 |
...
|       Melhoria do a provisionamento                      |          Aplique perfeitamente um pacote de provisionamento de uma unidade USB para os seus HoloLens                              |
|       Estado de instalação de aplicações                 |          Verifique o estado de instalação na aplicação Definições para apps que foram empurradas para HoloLens 2 via MDM               |
...
1. Junte o dispositivo ao Azure Ative Directory (Azure AD).
1. Utilize o Azure AD para inscrever o dispositivo no Microsoft Intune (ou noutro serviço MDM).
...
Saiba mais com o guia de [avaliação do Windows Autopilot para HoloLens 2.](https://docs.microsoft.com/hololens/hololens2-autopilot)
...
Alguns utilizadores partilham um dispositivo HoloLens com outros em ambiente de trabalho ou escola. Por isso, é importante que os utilizadores possam facilmente sem digitar nomes de utilizadores longos e senhas. Fast Identity Online (FIDO) permite que qualquer pessoa na sua organização (Azure AD insira perfeitamente os HoloLens sem introduzir um nome de utilizador ou senha.
...
Os pacotes de provisionamento permitem definir a configuração holoLens através de um ficheiro config e não através da experiência hololes fora da caixa. Anteriormente, os pacotes de provisionamento tinham de ser copiados para a memória interna dos HoloLens. Agora podem estar numa unidade USB para que sejam mais fáceis de reutilizar em vários dispositivos HoloLens e você pode providenciar dispositivos em paralelo. Os pacotes de provisionamento agora também suportam um campo para se inscrever na gestão do dispositivo para que não haja configuração manual após o provisionamento.

Para experimentá-lo:

1. Descarregue a versão mais recente do Windows Configuration Designer da loja Windows para o seu PC.
1. Selecione **Dispositivos HoloLens**  >  **Provision HoloLens 2 dispositivos**.
...
3. Ligue o dispositivo USB-C a quaisquer HoloLens recém-intermitentes. Em seguida, pressione os botões de potência **para baixo** do volume para aplicar o seu pacote de  +   provisionamento.

### <a name="line-of-business-application-install-status"></a>Estado de instalação de aplicações em linha de negócio

A implementação e gestão de aplicações de aplicativos MDM para aplicações de linha de negócios é fundamental para holoLens. Os administradores e os utilizadores precisam de visualizar o estado de instalação da aplicação para auditoria e diagnóstico. Nesta versão, adicionamos mais detalhes em **Definições**  >  **Contas**  >  **Aceder trabalho ou escola** Clique na sua  >  **conta**  >  **Info**.

### <a name="additional-csps-and-policies"></a>CSPs adicionais e políticas

Um [fornecedor de serviços de configuração (CSP)](https://docs.microsoft.com/windows/client-management/mdm/configuration-service-provider-reference?redirectedfrom=MSDN) é uma interface para ler, definir, modificar ou eliminar definições de configuração num dispositivo. Nesta versão, adicionamos suporte a mais políticas para aumentar os administradores de controlo que têm sobre dispositivos HoloLens implantados. Para a lista de CSPs apoiados por HoloLens, consulte [NetworkQoSPolicy CSP](https://docs.microsoft.com/windows/client-management/mdm/networkqospolicy-csp).

Novidades neste lançamento:

**Política CSP** 

O fornecedor de serviços de configuração de política permite à empresa configurar políticas em dispositivos Windows. Nesta versão, adicionámos novas políticas para os HoloLens, que estão listados aqui. Para saber mais, consulte [os CSPs de Política apoiados por HoloLens 2](https://docs.microsoft.com/windows/client-management/mdm/policies-supported-by-hololens2).  
...
Muitas aplicações do Windows suportam agora os modos escuros e claros. Os utilizadores do HoloLens 2 podem escolher o modo padrão para aplicações que suportam ambas. Após a atualização, o modo de aplicação predefinido é "escuro", mas pode facilmente alterar esta definição: Navegar para **Definições**  >  **Sistema**  >  **Cores**  >  **Escolha o seu modo de aplicação padrão**. 
...
Agora pode utilizar comandos de voz com qualquer aplicação no dispositivo. Para obter mais informações, consulte [use a sua voz para operar HoloLens](https://docs.microsoft.com/hololens/hololens-cortana). Consulte também [as línguas apoiadas para HoloLens 2](https://docs.microsoft.com/hololens/hololens2-language-support).  

### <a name="cortana-updates"></a>Atualizações cortana

A aplicação atualizada integra-se com o Microsoft 365 para ajudá-lo a fazer mais em todos os seus dispositivos (atualmente apenas em US-English). No HoloLens 2, cortana já não suporta certos comandos específicos do dispositivo, como ajustar volume ou reiniciar. Estas opções são agora suportadas pelos novos comandos de voz do sistema. Saiba mais sobre a nova aplicação Cortana no nosso [blog.](https://blogs.windows.com/windowsexperience/2020/02/28/cortana-in-the-upcoming-windows-10-release-focused-on-your-productivity-with-enhanced-security-and-privacy/)
...
- Consertou um bug onde Wi-Fi o streaming para holoLens foi interrompido periodicamente. Se uma aplicação indicar que necessita de um streaming de latência baixa, implemente a correção chamando a [função SetSocketMediaStreamingMode](https://docs.microsoft.com/windows/win32/api/socketapi/nf-socketapi-setsocketmediastreamingmode).
...
Esta atualização mensal de qualidade não contém quaisquer alterações notáveis porque a equipa estava a trabalhar na versão Holográfica do Windows 2004, como descrito anteriormente.

## <a name="windows-holographic-version-1903---april-2020-update"></a>Windows Holographic, versão 1903 - Atualização abril 2020
- Construção 18362.1059

**Modo escuro para aplicações suportadas** 

Muitas aplicações do Windows suportam o modo escuro e o modo de luz. Os clientes holoLens 2 podem agora escolher o modo padrão para aplicações que suportam ambos os esquemas de cores. Com base no feedback do cliente, definimos o modo de aplicação padrão para "escuro", mas pode facilmente alterar esta definição a qualquer momento: Navegar para **Definições > System > Colors** para encontrar **"Escolha o modo de aplicação padrão".**
...
## <a name="windows-holographic-version-1903---march-2020-update"></a>Windows Holographic, versão 1903 - março 2020 Atualização 
...
## <a name="windows-holographic-version-1903---january-2020-update"></a>Windows Holographic, versão 1903 - Atualização janeiro de 2020 
